...
const CabDriverSelectionPage = () => {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 sm:space-y-6">
      <div className="flex flex-col gap-4 rounded-[24px] border border-orange-100 bg-white p-4 shadow-sm sm:rounded-[32px] sm:p-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight text-gray-900 sm:text-4xl">Choose your Zahi driver</h1>
          ...
          {drivers.map((item, index) => {
            ...
            return (
              <motion.article
                ...
                className="rounded-[24px] border border-gray-100 bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-gray-900/5 sm:rounded-[30px] sm:p-5"
              >
                <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
                  ...
                  <div className="flex flex-wrap gap-3 sm:justify-end">
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-5 py-3 text-sm font-black text-orange-700 transition-all hover:bg-orange-100 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                    >
                      <FiImage />
                      Vehicle photos
                    </button>
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gray-900 px-6 py-3 text-sm font-black !text-white shadow-lg shadow-gray-900/10 transition-all hover:bg-black disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
                    >
                      {selecting ? (
      ...
      {gallery ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-950/80 p-3 backdrop-blur-sm sm:p-4">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-[24px] bg-white p-4 shadow-2xl sm:rounded-[32px] sm:p-5">
            <div className="mb-4 flex items-start justify-between gap-4 sm:items-center">
              <div>
                ...
                <h3 className="mt-1 text-xl font-black text-gray-900 sm:text-2xl">{gallery.title}</h3>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {gallery.photos.map((photo, index) => (
                <img key={`${photo}-${index}`} src={photo} alt={`${gallery.title} ${index + 1}`} className="h-56 w-full rounded-2xl object-cover sm:h-72" />
              ))}
            </div>
          </div>
        </div>
