...
async def get_flight_context(
    ...
    if role not in {"business_admin", "staff"}:
        raise HTTPException(status_code=403, detail="Flight workspace access is restricted.")
    ...
    if not tenant or tenant["business_type"] != "flight":
        raise HTTPException(status_code=403, detail="This workspace is not a flight workspace.")

    return FlightWorkspaceContext(
